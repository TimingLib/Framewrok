...
Saved_name=${Linux_version}_${Vivado_version_name}_20$(date +%y%m%d%H%M)
...
Usage(){
	...
	echo -e "\t[NAME]	(Optional parameters)The name of the image that you want to save/restore.Like:image_clone.sh -r/s [NAME]"
} 
...
cp -n /boot/efi/EFI/redhat/grub.conf /boot/efi/EFI/redhat/grub.conf.bak
...
Mount_server(){
ping -c 1 SH-RD-TimingLib 1>/dev/null
if [ $? = 0 ]
	then
...
if [ $# -eq 0 ]
	then	
		...
		while getopts :r:sh opt ;
			do
				case $opt in	
				s)	echo -e "\e[0mDisk to Image (\e[31m${Saved_name}\e[0m) as image_name"
			    		Mount_server
				...
				\:)	echo -e "Image to Disk (\e[31mFirst_image\e[0m as default image_name)"
			    		Mount_server
					Select_name
					Restore_disk ${name_image} ;;
				r)	echo -e "Image to Disk (\e[31m$OPTARG\e[0m as default image_name)"
					Restore_disk $OPTARG ;;
				h)	Usage 
				 	exit 0 ;;
				\?)	echo -e "\e[31mInvalid Option/Miss the args\e[0m" 
	          	   		Usage
					exit 21 ;;
				esac
		done
